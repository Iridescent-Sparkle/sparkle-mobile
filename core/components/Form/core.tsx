...
function createForm(target?: any, ...names: string[]) {
  ...
  function validateFields({ showMsg = true } = {}) {
    // 对rules进行排序
    const entries = Object.entries(rulesOrder).sort(([, aValue]: [string, number], [, bValue]: [string, number]) => {
      if (aValue === 0)
        return 1
      if (bValue === 0)
        return -1
    ...
    return new Schema(sortedRules).validate(values).then(() => {
  ...
  function validateField(name: string) {
    return new Schema({ [name]: rules[name] }).validate({ [name]: values[name] }).then(() => {
      return getFieldsValue()
    }).catch(({ errors }) => {
      // $.msg(errors[0].message)
      return Promise.reject(errors)
...
function FormItem(props: FormItemProps) {
  ...
  useEffect(() => {
    // 更新规则
    if (rules) {
      form[setRulesSy]((formRules: { [x: string]: any }, rulesOrder: { [x: string]: any }) => {
...
const Form: React.FC<FormProps> & FormType = (props: FormProps) => {
  ...
  // 表单值监听
  useEffect(() => {
    const unValuesListen = onChange ? form[valuesListenSy](onChange) : undefined
    return () => {
